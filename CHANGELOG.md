...
**The operator's `AC-107` Decision 9 is delivered in full.** The binding is generated by the backend, persisted with the `Plan`, **required** on every authorization, and compared **server-side** — against a hash recomputed from persisted content, never against the stored field, so a record whose hash and content ever disagreed fails closed. "Backend-generated" is a module boundary rather than a convention: `@foundry/contracts` ships to the browser and holds no `node:crypto`, so the producer is unreachable from the client. A client may state which hash it *read* and be refused if that disagrees; the value written is always the backend's own, and the client's field is deleted rather than carried.

**Single-use, two ways.** A second `Plan.Authorize` is refused by name. Spend is derived from persisted truth — a real `AgentRun` for the stage — not from a mutable flag, so a run that started and failed still counts as spent (`F-124`).

**Stated plainly: there is no real invocation for this gate to prevent yet.** No runtime is wired, nothing is spent, and no Claude Code was invoked in building it. What is proven now is that the gate refuses exhaustively, permits only under a valid authorization, and that neither reading it nor being refused by it changes anything. `F-114`'s full claim becomes demonstrable at `AC-111`.

**The adversarial case was performed, not argued.** The persisted plan was tampered with directly in the database and the API restarted; the recomputed hash no longer matched the authorization's binding and the gate refused, naming the binding as the reason.

**Two defects found before commit.** The `F-113a` boundary assertion was defeated by the comment documenting that very boundary — the second time that class of mistake appeared — and is now asserted over import statements. And a test harness's default parameter meant the "uncredentialed caller" case silently sent the operator's token: the product was correct, but the test had been asserting a fact it never exercised.

`Plan.Authorize` joins the closed vocabulary and `operator.execution_authorized` joins the runtime event vocabulary, each at the rung that produces it. Amendments recorded in `domain-model.md` and `event-model.md`.

**Gates:** typecheck 8/8 · lint clean · build clean · **1324 passed / 92 files / 0 failures** (up from 1239). `v1-canonical-run.json` re-checked **byte-identical to `HEAD`**. Recorded in `docs/evidence/ac-110/authorization-gate-record.md`.

**`AC-110` is implemented but not closed** — it stops for the operator's review. `AC-111` not started, no real execution performed, no money spent, no baseline work, no Finding 6 work, no NAS District file touched.
