...
**Date:** 2026-08-03
**Status:** Implemented, then **corrected at operator contract review (2026-08-03)**. Awaiting final operator review. The rung is not closed.
...
| `ExecutionAuthorizationSchema` | `packages/contracts/src/authorization.ts` | A multi-use authorization, a build-wide authorization, an **absent or unbounded** budget, an authorization that outlives the plan it was granted against |
| `ExecutionAuthorizationDraftSchema` | `packages/contracts/src/authorization.ts` | A half-filled pre-authorization being mistaken for permission — the two schemas are mutually exclusive |
...
### Plan binding — corrected at operator review

`planRevision()` (formerly `fingerprintPlan`) is a **revision/change indicator only**. It is a non-cryptographic FNV hash, computable in a browser, dependency-free because `packages/contracts` is imported by the client bundle. It detects that a plan changed. It is **not** a security boundary and is not represented as one.

**The authoritative execution binding is amended into `AC-110`:** a **backend-generated SHA-256 hash of canonical persisted plan content**, stored with the `Plan` and compared **server-side**. A client-supplied value is never accepted as the binding. Recorded in the build ladder § AC-110 and in `v1.1-acceptance.md` as `F-113a`. Not implemented here — `AC-107` is contract-only.

`authorizesPlan()` returns **every** mismatch at once; its `plan_modified` result reports revision drift, not an execution decision.

### Real-execution allocation

The `claude_code` runtime may be planned only for **`backend_implementation`**, and for no more than one stage. The authoritative rule is narrower than "at most one" — `domain-model.md` → AgentRun invariants names the stage: *"exactly one `AgentRun` in V1 uses `runtimeType: claude_code` (the `backend_implementation` stage, per `v1-scope.md`)"*. A plan allocating `claude_code` to `scaffold` is refused even as the only such stage.

### Budget

`maxBudgetUsd` is **required** on every issued authorization: positive, finite, capped at **$25** for V1.1. An authorization with no spend ceiling is unrepresentable. An incomplete pre-authorization uses the distinct, non-authorizing `ExecutionAuthorizationDraftSchema`.
...
| **Backend-generated SHA-256 plan binding** — computed over canonical persisted plan content, stored with the `Plan`, compared server-side. Required by `F-113a`; `planRevision` may not substitute for it | `AC-110` |
...
`pnpm typecheck` 8/8 · `pnpm lint` clean · `pnpm build` clean · `pnpm -r run test` → **1121 passed / 91 files / 0 failures** (up from 1033), after the operator-review corrections.

Contract package alone: **151 tests**, covering every positive and negative case `F-107` names — too-short and over-long objective, disallowed workspace, R3+ risk, unknown stage name, malformed plan, malformed authorization.
