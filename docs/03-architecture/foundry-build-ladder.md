...
| **Verification record** | ⚠️ **Open — not closed.** Authorized and executed 2026-08-01 against `1b1689a`; full report at `docs/evidence/fbl-035/v1-acceptance-report.md`. All six gates ran **together as one suite**: typecheck 8/8, lint clean, production build, **791** unit/integration, **363** browser (3 viewports), **16** performance (4 configurations) — 0 failures. Every F-01–F-12 and V-01–V-08 requirement is mapped to the tests that actually prove it, verified by reading them rather than trusting ID annotations. Exclusions confirmed unimplemented (notably `V1RiskClassSchema` makes R3–R5 *unrepresentable*, and the command bar is a closed six-command union with no text input); documentation cross-checked against implementation. **Three findings.** (1) `shell-realtime-connection.spec.ts` — the browser-level proof of **F-10**, a mandatory requirement — had been skipped on every run since FBL-026 because it is gated on a backend-mode build; its screenshot baselines had never been created. It was executed in backend mode for this report and **passes at all three viewports**, every functional assertion passing before any baseline existed. (2) F-12's real controlled Claude Code run was **not** re-executed: the FBL-028 authorization was narrow and is exhausted, so re-running requires fresh authorization; the retained FBL-028 evidence stands. (3) **Safari/WebKit had never been exercised by automation** although `v1-acceptance.md` names it a target browser. Running it surfaced **7 failures across 3 issues**: `<button>` elements unreachable by Tab, a camera-settling moving-target race in Lighthouse pointer selection, and canvas pixel readback at 5120×1440. No fix was attempted — each would reopen its owning rung (FBL-006, FBL-015, FBL-014), which is the operator's scope decision. **Update 2026-08-01:** the operator observed the application in **real macOS Safari** at 5120×1440 (`docs/evidence/fbl-035/real-safari-observation.md`) and reported keyboard, Lighthouse selection, rendering, and the primary journey all **PASS**. All three WebKit issues are therefore classified as non-defects — configuration/automation-dependent (3a), automation-only moving-target race (3b), and test-instrumentation behaviour (3c) — and FBL-006/FBL-014/FBL-015 are **not** reopened. F-12 was **re-executed live** under a one-run operator authorization and verified (write confinement clean, independent validation 12/12); that authorization is now spent (`docs/evidence/fbl-035/f12-verification/`). Two stale operator-facing strings were narrowly corrected. **Two blocking findings remain.** *Finding 4:* `jump to world object` is specified in `interface-model.md` § "Bottom event timeline" and named in FBL-009's objective, but **was never implemented** — FBL-009 hard-stopped before the 3D mapping and FBL-021 never claimed it, and two tests pin its absence as expected. This breaks the Definition-of-done condition "documentation matches implementation". *Finding 5:* the acceptance rerun is **not clean** — `shell-selection.spec.ts:23` fails intermittently on a camera-settling moving-target race, now reproduced in Chromium as well as WebKit. **This rung does not close** while those items are outstanding (field 12), and the operator's personal end-to-end journey (field 10) has not been signed off. **V1 is not declared complete.** |
