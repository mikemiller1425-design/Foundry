...
- **Gate, stated precisely (corrected 2026-08-04):** the operator approved this rung on the basis of the assistant's implementation report, automated verification, and the assistant's **live-verification summary** — **not** by personally performing the checklist in the recorded environment. The approval is a reviewed governance act and the rung is closed; the operative technical evidence is the named automated tests and the recorded live run, both reproducible at the commit. The correction is appended to the observation record. **This does not contribute to definition-of-done § 7.8** — "the operator personally performs the § 3 journey" — which remains owed at `AC-120`.
